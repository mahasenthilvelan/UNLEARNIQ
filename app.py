...
.metric-value {
    font-family:'Orbitron',monospace; font-size:1.65rem;
    font-weight:700; color:#00F5FF;
}
.metric-label {
    font-size:0.75rem; color:#6AABFF;
...
.model-name {
    ...
    color:#8B5CF6; letter-spacing:0.1em; margin-bottom:0.5rem; text-transform:uppercase;
}
...
# SESSION STATE
# ══════════════════════════════════════════════════════
for _k, _v in {"phase":"splash","df_raw":None,"selected_users":[],"results":None}.items():
...
def validate_and_load(df):
    """Return (text_col, label_col, ok, message)"""
    # text column
    ...
    if 'toxic' in df.columns:
        ...
        return None, None, False, "No label column found. Need `toxic`, `class`, or `label`."
...
# ══════════════════════════════════════════════════════
# PIPELINE  (matches every Colab step exactly)
# ══════════════════════════════════════════════════════
def run_pipeline(df_raw, selected_users, pb, st_txt):
    ...
    # ── MODEL COMPARISON (Colab addition) ────────────
    ...
    # ── STEP 8-9: Collect target user data ───────────
    ...
    # ── STEP 10 + 23: Unlearn (multi-criteria) ───────
    st_txt.markdown('<div class="step-badge">STEP 5 — MACHINE UNLEARNING (RETRAIN)</div>', unsafe_allow_html=True)

    # Step 23: remove only toxic rows of selected users
    user_data_toxic = df[(df['UserId'].isin(selected_users)) & (df['label'] == 1)]
    ...
    # Step 10.1: zero out user-word weights
    ...
    imp_after = pd.DataFrame({
        'word': tfidf_u.get_feature_names_out(),
        'weight': unlearn_model.coef_[0]
    })
...
# ══════════════════════════════════════════════════════
    ...
    with col:
        ...
        uploaded = st.file_uploader(
            ...
            type=["csv"],
            help="Kaggle Toxic Comments: comment_text + toxic  |  or  tweet + class"
        )
        ...
        if uploaded:
            ...
            if not ok:
                ...
                st.dataframe(df_raw[['comment_text' if 'comment_text' in df_raw.columns else 'tweet', label_col]].head(5), use_container_width=True)
...
# ══════════════════════════════════════════════════════
    ...
    n_rows        = len(df_raw)
    all_users     = ['user_' + str(i) for i in range(1000)]
    rows_per_user = max(1, n_rows // 1000)
    ...
    with c2: st.markdown(f'<div class="metric-card"><div class="metric-value">1,000</div><div class="metric-label">Unique Users</div></div>', unsafe_allow_html=True)
    with c3: st.markdown(f'<div class="metric-card"><div class="metric-value">~{rows_per_user}</div><div class="metric-label">Rows / User</div></div>', unsafe_allow_html=True)
    ...
    <div class="user-box">
        ...
        <div style="color:#4A7AAA;font-size:0.86rem;margin-top:0.3rem;">
            Select one or more users. Only their <b style="color:#FF5252">toxic</b> rows are removed
            before retraining (multi-criteria unlearning — Step 23).
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    selected = st.multiselect(
        ...
        default=default_sel,
        help="Pick one or many. All selected users' toxic rows will be excluded from retraining."
    )

    if selected:
        approx = len(selected) * rows_per_user
...
# ══════════════════════════════════════════════════════
    ...
    display:flex;justify-content:space-between;flex-wrap:wrap;gap:0.4rem;">
        ...
        <span style="color:#6AABFF">
            <b style="color:#00F5FF">{r['user_sample_count']:,}</b> total user rows &nbsp;|&nbsp;
    ...
    cols_map = [mc1, mc2, mc3]
    for idx, (name, acc) in enumerate(r["model_results"].items()):
        is_winner = (name == best_model)
        badge     = '<div class="model-winner-badge">⭐ BEST MODEL</div>' if is_winner else ""
        winner_cls = "model-card-winner" if is_winner else ""
        cols_map[idx].markdown(
            f'<div class="model-card {winner_cls}">'
            f'<div class="model-name">{name}</div>'
            f'<div class="model-acc">{acc:.3f}</div>'
            f'{badge}</div>',
            unsafe_allow_html=True
        )
    ...
    fig_mc = go.Figure(go.Bar(
        ...
        marker=dict(
            color=list(r["model_results"].values()),
    ...
    # MIA Before vs After bar
    fig_mia = go.Figure()
    fig_mia.add_trace(go.Bar(name='Before', x=['MIA'], y=[r["mia_before"]], marker_color='#FF4D4D', width=0.25))
    fig_mia.add_trace(go.Bar(name='After',  x=['MIA'], y=[r["mia_after"]],  marker_color='#00F5FF', width=0.25))
    ...
    with g_col:
        fig_g = go.Figure(go.Indicator(
            ...
            title={'text':'Final Enhanced Score',
                   'font':{'color':'#6AABFF','family':'Orbitron','size':13}},
            gauge={
    ...
    with t_col:
        st.markdown("<br>", unsafe_allow_html=True)
        mia_signal = max(0.0, r["mia_before"] - r["mia_after"])
        st.markdown(f"""
        <table class="score-table">
          ...
          <tr><td>MIA Signal &nbsp;×0.25</td><td>{mia_signal:.4f}</td></tr>
